...
class RcXmlMigration:
    ...
    def apply_rewrites(
        ...
        """Apply the rewrites of every rc.xml version above from_version; True when any did"""
        user_root = user_et.getroot()
        changed = False
        ...
        for version in sorted(REWRITES):
            ...
            log.info(f"Applying rc.xml version {version} rewrites")
            for rewrite in REWRITES[version]:
                changed |= rewrite.apply(user_root)
