...
namespace Budgie {
	...
	*/
	public class RavenPluginItem : Gtk.Grid {
		/**
		...
		*/
		...
		/**
		* Construct a new PluginItem for the given widget
		*/
	...
	public class RavenWidgetChooser : Gtk.Dialog {
		...
		public RavenWidgetChooser(Gtk.Window parent) {
			Object(use_header_bar: 1,
				modal: true,
				title: _("Choose an widget"),
				transient_for: parent);

			Gtk.Box content_area = get_content_area() as Gtk.Box;

			this.add_button(_("Cancel"), Gtk.ResponseType.CANCEL);
			button_ok = this.add_button(_("Add widget"), Gtk.ResponseType.ACCEPT);
		...
		*/
		void row_selected(Gtk.ListBoxRow? row) {
			...
			this.applet_id = ((PluginItem) row.get_child()).plugin.get_module_name();
		}
